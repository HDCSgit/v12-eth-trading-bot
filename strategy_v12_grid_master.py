#!/usr/bin/env python3
"""
V12-Grid-Master: 震荡行情救命版（ML+网格一键融合）
一针见血解决：震荡市不交易、胜率27%的问题
"""
...
class EnhancedFeatureEngineer:
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['target'] = np.where(df['future_return'] > 0.005, 1, 
                               np.where(df['future_return'] < -0.005, 0, -1))
...
class V12GridMasterTrader:
    ...
    def is_sideways(self, df: pd.DataFrame) -> bool:
        ...
        # 放宽到 BB < 0.10 且 RSI 35~65
        return bb_width < 0.10 and 35 < rsi < 65
    ...
    def open_grid(self, side: str, entry_price: float, qty: float, atr: float):
        ...
        for i, mult in enumerate([1.0, 2.0, 3.0]):
            if side == 'BUY':
                tp_price = entry_price + (atr * mult * 1.4)
            else:
                tp_price = entry_price - (atr * mult * 1.4)
                ...
                if (self.position_side == 'BUY' and price >= level['price']) or \
                   (self.position_side == 'SELL' and price <= level['price']):
                    ...
                    if all(l['filled'] for l in self.grid):
                        self.stats['wins'] += 1
                        # 返还剩余保证金
                        self.balance += self.margin * 0.5  # 简化处理
                        self.position_side = None
                        self.grid = []
        ...
        for i in range(30, len(test_df)):
            ...
            if not self.position_side:
                ...
                # 震荡市：强制开网格
                    ...
                    # 靠近上轨做空，靠近下轨做多
                    if close > bb_mid + (bb_upper - bb_mid) * 0.3:
                        action = 'SELL'
                    elif close < bb_mid - (bb_mid - bb_lower) * 0.3:
                        action = 'BUY'
...
def main():
    ...
    print("\n" + "=" * 70)
    print("🚀 V12-Grid-Master（震荡市救命版）回测报告")
    print("=" * 70)
